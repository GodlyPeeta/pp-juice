...
def pp_calc(aim, speed, b, misses, c100, c50, used_mods=mods(), combo=0xFFFF, score_version=1, c300=0xFFFF):
    ...
    if used_mods.fl:
        aim_value *= 1.0 + 0.35 * min(1.0, total_hits / 200.0) + ((0.3 * min(1, (total_hits - 200) / 300.0) + (
            (total_hits - 500) / 1200.0 if total_hits > 500 else 0)) if total_hits > 200 else 0)
    ...
    class d_obj:
        def __init__(self, base_object, radius, prev):
            ...
            if (prev != None):
                ...
                if (self.ho.h_type != 3):
                    # Calculate speed
                    self.strains[0] = prev.strains[0] * math.pow(consts.decay_base[0],
                                                                 self.delta_time / 1000.0) + self.calculate_speed(
                        prev) * consts.weight_scaling[0]
                    # Calculate aim
                    self.strains[1] = prev.strains[1] * math.pow(consts.decay_base[1],
                                                                 self.delta_time / 1000.0) + self.calculate_aim(prev) * \
                                      consts.weight_scaling[1]
        ...
        def calculate_aim(self, prev):
            ...
            if (prev != None):
                if (self.angle != None and self.angle > consts.aim_angle_bonus_begin):
                    scale = 90
                    angle_bonus = math.sqrt(max(prev.jump_distance - scale, 0) * math.pow(
                        math.sin(self.angle - consts.aim_angle_bonus_begin), 2) * max(self.jump_distance - scale, 0))
            ...
            if (self.angle != None and self.angle < consts.speed_angle_bonus_begin):
                ...
                if (self.angle < math.pi / 2):
                    ...
                    if (distance < 90 and self.angle < math.pi / 4):
                        ...
                        angle_bonus += (1 - angle_bonus) * min((90 - distance) / 10, 1) * math.sin(
                            ((math.pi / 2) - self.angle) / (math.pi / 4))
            return (1 + (speed_bonus - 1) * 0.75) * angle_bonus * (
                        0.95 + speed_bonus * math.pow(distance / consts.single_spacing, 3.5)) / strain_time
...
def ppcalculate(ac, co, mi, mo, link):
    print("ASFFAF"+mo)
    feature = True
    c100 = 0
    c50 = 0
